...
void GameScene::Draw()
{
  ...
  if(state==0)
  {
    ...
    batch->Draw(tex,Vec2D(ChorusApp::m_VscrW/2,ChorusApp::m_VscrH/2));

    batch->setTint(0.0f,1.0f,1.0f,1.0f);
    batch->Draw(tex,Vec2D(ChorusApp::m_VscrW*0.8f,ChorusApp::m_VscrH/2));
  ...
  //if(ChorusApp::curTime()-lastRender>1000)
  {
    ...
    if(swt==0)
      batch->Draw(target2.tex,Vec2D(ChorusApp::m_VscrW/2,ChorusApp::m_VscrH/2),Vec2D(1.0f,-1.0f));
    if(swt==1)
      batch->Draw(target.tex,Vec2D(ChorusApp::m_VscrW/2,ChorusApp::m_VscrH/2),Vec2D(1.0f,-1.0f));
    batch->End();
